namespace StrideCommunity.ImGuiDebug
{
    ...
    public class ImGuiSystem : GameSystemBase
    {
        ...
        public ImFontPtr FontDefault { get; private set; }
        public ImFontPtr FontLarge { get; private set; }
        ...
        public IntPtr AddTexture(Texture texture)
        {
            ...
            return new IntPtr(_textures.Count);
        }
        ...
        unsafe void CreateFontTexture()
        {
            ...
            if (Content.Exists("Fonts/FR_MainFont"))
            {
                using var stream = Content.OpenAsStream("Fonts/FR_MainFont");
                using var reader = new BinaryReader(stream);

                var fontData = reader.ReadBytes((int)stream.Length);
                fixed (byte* p = fontData)
                {
                    FontDefault = io.Fonts.AddFontFromMemoryTTF((IntPtr)p, 32, 32);
                    FontLarge = io.Fonts.AddFontFromMemoryTTF((IntPtr)p, 64, 64);
                }
            }
            else
            {
        ...
        void RenderDrawLists(ImDrawDataPtr drawData)
        {
            ...
            for (int n = 0; n < drawData.CmdListsCount; n++)
            {
                ...
                for (int i = 0; i < cmdList.CmdBuffer.Size; i++)
                {
                    ...
                    if (cmd.TextureId != IntPtr.Zero && (int)cmd.TextureId <= _textures.Count)
                    {
                        imShader.Parameters.Set(ImGuiShaderKeys.tex, _textures[(int)cmd.TextureId - 1]);
                    }
                    else
                    {
                        imShader.Parameters.Set(ImGuiShaderKeys.tex, fontTexture);
